...
$("input[type=radio]").change(function(){ //code to check whether the sum of selected option is 6 for TVRLS application, code by Raj
  ...
  if($(this).attr('class') == "grid-required op_radio checkSum6")
  {
      ...
      var userSelectedVal = (selectedValArray[2] -1); //Logic to identify scale text from the radio button id
     // alert(userSelectedVal);
      totalSumSelectOpt = userSelectedVal;
      var hv = $('#hidden_'+selectedValArray[1]).attr("value"); //get the question id's stored in hidden field
      ...
      for (qtnCount = 0; qtnCount<questionArray.length; qtnCount++)
      {
        ...
        var srdBtnId = $('input[type=radio][name=_'+questionArray[qtnCount]+']:checked').attr('id'); //Get the Id of selected radio button for this preamble
        if (typeof srdBtnId  !== "undefined")//When I choose only 1st question the loop runs through all questions in the preamble and returns undefined and hence we are checking
        {
           //alert(srdbtnid);
           if(srdBtnId != selectedVal)
           {
              ...
              {
                    alert("Please fill the survey correctly. The sum of all three sub-questions (a), (b) and (c) should not be less than 6 in question ");
                      $(this).attr('checked', false);
              }
