...
$optionth=(isset($ques->optionth)) ? $ques->optionth : array();
...
<div class="panel-body question-options" data-nottouch="{{$datanottouch}}">
 ...
 <tr>
   ...
   @if(count($optionth)>0)
   <th class="question-dimension" style="width:5%"></th>
   <th class="question-dimension" style="width:30%"></th>
   @foreach($optionth as $optionthi)
   <th class="text-center question-dimension" style="width:6%;text-align:center">{!!$optionthi!!}</th>
   @endforeach
   @endif
   @if(count($questionArrayTmp)==2)
   <th class="question-dimension" style="width:22%"></th>
   @endif
 </tr>
@if(count($c)>0)
   <?php //$count=1; ?>
   ...
   <tr class="data-question-type-grid">
    @if($checkSum6Flag >= 2)
		<td class="question-dimension qcoun"> {{$arrayAlpCount[$count-1]}} </td>
        <td class="question-dimension"> {!!$questionTextPart1!!} </td>
    @else
      <td class="question-dimension qcoun"> {{$count}} </td>
      <td class="question-dimension"> {!!$questionTextPart1!!} </td>
    @endif

   <?php
     $i_options=(isset($optext[$ques_id])) ? $optext[$ques_id] : array();
     $option_count=1;
   ?>
...
@endif
 ...
 <div class="message">
   <span><strong>Ooops!</strong> You must make a selection</span><div>
</div>
