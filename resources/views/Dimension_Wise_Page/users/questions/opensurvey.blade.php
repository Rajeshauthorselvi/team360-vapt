...
<div class="col-sm-12  col-md-8 col-md-offset-2 col-xs-12 welcome-txt" id="question-container">
    ...
    <form action="{{ route('ostore', config('site.survey_slug')) }}" method="POST" id="survey-user-form">
        @csrf
      ...
      @if(count($questions)>0)
          ...
          @foreach($questions as $key=>$question)
            ...
            @if($question->question_type=="grid")
                ...
                @include('default.users.questions.gridquestions')

            @else

                @include('default.users.questions.alltypequestions')
...
<style type="text/css">#form-save{display: none;}</style>
@if($rcount>0)
<style type="text/css">
