                            ...
                            @if (count($user_survey_info) > 0)
                                @foreach ($user_survey_info as $key => $usinfo)
                                    <tr>
                                        ...
                                        @endif
                                        {{-- <td data-label="Survey Start">
					<span class="allocate_table_content">
						{{date("d/m/Y", strtotime($usinfo->start_date))}}
					</span>

				</td>
				<td data-label="Survey End">
					<span class="allocate_table_content">
						{{date("d/m/Y", strtotime($usinfo->end_date))}}
					</span>
				</td> --}}
                                            ...
                                            <td class="text-center " data-label="Respondents">
                                                <span class="allocate_table_content">
                                                    @if ($usinfo->participant_rater_manage == 1 and $usinfo->respondent_id == 0)
                                                        ...
                                                        @if ($survey_exists)
                                                            <a
                                                                href="{{ route('manage-email.index', [config('site.survey_slug')]) }}">
                                        ...
                                        $user_exam_info = [config('site.survey_slug'), 'rater=' . $rater_id, 'participant=' . $participant_id, 'rater_type=' . $usinfo->rater];
