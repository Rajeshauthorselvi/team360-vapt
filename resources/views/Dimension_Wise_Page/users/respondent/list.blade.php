    ...
    <div class="row ">
        <div class="col-xs-12 col-sm-12 col-md-12 list-page">
            ...
            <div class="action-panel pull-right">
                <a href="{{ route('manage-respondent.create',[config('site.survey_slug')]) }}" class="btn btn-submit"><i
                ...
                ?>
                        ...
                        <td data-label="Edit" class="icon action-btn action">
                            <span class="allocate_table_content">
                                ...
                                <a href="{{ route('manage-respondent.edit',[config('site.survey_slug'),0,'user_id='.$result->id]) }}"
                                    data-toggle="tooltip" title="Edit respondent details"
                                    class="btn btn-info icon_symbol"><span class="fa fa-edit"></span></a>
                                @if($result->notify_email_date !=null)
                                <a href="{{ route('resend.resendaccess',['respondent_id'=>$result->id,'survey_id'=>$survey_id]) }}"
                                    data-toggle="tooltip" title="Resend Access" class="btn btn-info icon_symbol"><span
                                        class="fa fa-refresh"></span></a>
                                @endif


                                <form class="form-inline" role="form" method="GET"
                                    action="{{URL::route('manage-respondent.show',[config('site.survey_slug'),0])}}">
                                    <input type="hidden" name="survey_id" value="<?= $survey_id?>">
                                    <input type="hidden" name="action" value="reopensurvey">

                                    <input type="hidden" name="respondent_id" value="<?= $result->id?>">
                                    @if($result->rcount > 1)
                                    <button class="reopen-survey btn btn-info icon_symbol" type="button"
                                ...
                                <form class="form-clear" role="form" method="GET"
                                    action="{{URL::route('manage-respondent.show',[config('site.survey_slug'),0])}}">
                                    ...
                                    <input type="hidden" name="respondent_id" value="<?= $result->id?>">
                                    @if($result->rcount > 1)
                                    <button class="clear-response btn btn-info icon_symbol" type="button"
                                ...
                                <form
                                    action="{{ route('manage-respondent.destroy', [config('site.survey_slug'), $result->id]) }}"
                                    method="POST" class="del_form">
                                    @csrf
                                    @method('DELETE')
                                    ...
                                    @if($result->rcount > 1)
                                    <input type="hidden" name="check_response_data" value="true"
                                        class="check_response_data">
                                    @else
                                    <input type="hidden" name="check_response_data" value="false"
                                        class="check_response_data">
                                    @endif

                                    <button class="delete-user-survey btn btn-info icon_symbol" type="button"
                    ...
                    @else
                    <tr>
                        <td class="text-center" colspan="7">No Respondents found!</td>
                    </tr>
                    @endif
