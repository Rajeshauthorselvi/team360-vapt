...
foreach($largestvalue as $questionid=>$diff){
$question_dimension_large[$questionid]=DB::table('questions')->where('id',$questionid)->value('question_dimension');
}
}
?>
...
<br/>      
<b>Top 5 Diverging Statements</b>
...
<br/>
<br/>
<b>Top 5 Converging Statements</b>
<table class="table table-striped table-bordered dt-responsive nowrap survey-table" width="100%" cellspacing="0">
 <thead>
    <tr>
      <th>S.no</th>
...
@if(count($largestvalue)!=0)
<?php $s_no2=1; ?>
@foreach($largestvalue as $question_id=>$diff)
<tr>

<td>{{$s_no2}}</td>

<td>{{$question_dimension_large[$question_id]}}</td>
