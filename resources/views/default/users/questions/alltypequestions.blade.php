...
<li class="group active @if($no==1) {{'focus'}} @endif" data-id="{{$no}}">
    ...
    ?>
        ...
        <div class="panel-body question-options" data-nottouch="{{$datanottouch}}">
            ...
            <div class="count_message {{ $status }}" style="{{ $display }}"><span><strong>Ooops!</strong> You must write
                    minimum 20 words.</span></div>
            @endif
            ...
            @endif --}}
            @if ($survey_id==37)
            <div class="error-message-check hidden"><span><strong>Ooops!</strong> You must need to select minumum 2
                    selection</span></div>
            @endif
