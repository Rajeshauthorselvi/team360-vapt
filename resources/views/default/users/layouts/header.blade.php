...
<div class="welcome-strip">
	<div class="container">
		...
		@if(Auth::check())
			<div class="col-md-12 ">
  <a href="{{URL::route('user.dashboard',[config('site.survey_slug')])}}" title="Home" ><span class="glyphicon glyphicon-home" ></span></a> &nbsp;
  				<?php $current_slug=explode('/',Request::path()) ?>
  				@if ($current_slug[1]=="user-dashboard" || $current_slug[1]=="change-password")
	  				Welcome <strong style=" text-transform: capitalize">{{Auth::user()->fname.'  '.Auth::user()->lname}}</span></strong>
  				@else
  					@if(config('site.survey_slug')=='bvcboard')
  						@if(Session::get('rater_msg')=='Self Assessment')
  							BOARD EVALUATION
  						@else
  							{{ Session::get('rater_msg') }}
  						@endif
  					@else
  						{{ Session::get('rater_msg') }}
  					@endif
